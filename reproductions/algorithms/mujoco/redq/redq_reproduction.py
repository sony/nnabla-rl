...
def run_training(args):
    ...
    config = A.REDQConfig(gpu_id=args.gpu, fix_temperature=args.fix_temperature)
    redq = A.REDQ(train_env, config=config)

    hooks = [iteration_num_hook, save_snapshot_hook, evaluation_hook]
    redq.set_hooks(hooks)

    total_iterations = select_total_iterations(args.env) if args.total_iterations is None else args.total_iterations
    redq.train_online(train_env, total_iterations=total_iterations)
