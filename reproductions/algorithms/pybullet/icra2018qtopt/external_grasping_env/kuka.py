# This file is a fork from an Apache License 2.0 project named google-research:
# https://github.com/google-research/google-research/blob/master/dql_grasping/kuka.py (#6c51450)
...
class Kuka:
    def __init__(self, urdfRootPath="", timeStep=0.01, clientId=0, ikFix=False, returnPos=True):
        ...
        self.trayUid = pybullet.loadURDF(
            tray_path, [0.64, 0.075, -0.19], [0.0, 0.0, 1.0, 0.0], physicsClientId=self.cid
        )
        self.reset()

    def reset(self, base_pos=None, endeffector_pos=None):
        ...
        pybullet.resetBasePositionAndOrientation(
            self.kukaUid, base_pos, [0.000000, 0.000000, 0.000000, 1.000000], physicsClientId=self.cid
        )
        self.jointPositions = [
            0.006418,
            0.413184,
        ...
        for jointIndex in range(self.numJoints):
            pybullet.resetJointState(
                self.kukaUid, jointIndex, self.jointPositions[jointIndex], physicsClientId=self.cid
            )
    ...
    def getObservation(self):
        observation = []
        state = pybullet.getLinkState(self.kukaUid, self.kukaEndEffectorIndex, physicsClientId=self.cid)
        if self.ikFix:
        ...
        if self.useInverseKinematics:
            ...
            if self.useNullSpace == 1:
                ...
                if self.useOrientation == 1:
                    ...
                    jointPoses = pybullet.calculateInverseKinematics(
                        self.kukaUid, self.kukaEndEffectorIndex, pos, maxNumIterations=1, physicsClientId=self.cid
                    )
            if self.useSimulation:
                ...
                for i in range(len(jointPoses)):
                    pybullet.resetJointState(self.kukaUid, i, jointPoses[i], physicsClientId=self.cid)
            # Move fingers.
