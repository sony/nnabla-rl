...
class TestFileWriter:
    def test_write_scalar(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            writer = FileWriter(outdir=tmpdir, file_prefix="evaluation_results")
            writer.write_scalar(1, test_results)

            file_path = os.path.join(tmpdir, "evaluation_results_scalar.tsv")
            this_file_dir = os.path.dirname(__file__)
            test_file_dir = this_file_dir.replace("tests", "test_resources")
            test_file_path = os.path.join(test_file_dir, "evaluation_results_scalar.tsv")
    ...
    def test_write_histogram(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            file_path = os.path.join(tmpdir, "evaluation_results_histogram.tsv")
            this_file_dir = os.path.dirname(__file__)
            test_file_dir = this_file_dir.replace("tests", "test_resources")
            test_file_path = os.path.join(test_file_dir, "evaluation_results_histogram.tsv")
            self._check_same_tsv_file(file_path, test_file_path)
    ...
    def _check_same_tsv_file(self, file_path1, file_path2):
        # check each line
        with open(file_path1, mode="rt") as data_1, open(file_path2, mode="rt") as data_2:
