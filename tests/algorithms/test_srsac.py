...
class TestSRSAC(object):
    def setup_method(self, method):
        nn.clear_parameters()

    def test_algorithm_name(self):
        dummy_env = E.DummyContinuous()
        srsac = A.SRSAC(dummy_env)

        assert srsac.__name__ == 'SRSAC'
    ...
    def test_run_online_training(self):
        ...
        srsac.train_online(dummy_env, total_iterations=10)

    def test_run_offline_training(self):
        """Check that no error occurs when calling offline training."""
        batch_size = 5
        dummy_env = E.DummyContinuous()
        config = A.SRSACConfig(batch_size=batch_size)
        srsac = A.SRSAC(dummy_env, config=config)

        experiences = generate_dummy_experiences(dummy_env, batch_size)
        buffer = ReplayBuffer()
        buffer.append_all(experiences)
        srsac.train_offline(buffer, total_iterations=10)
    ...
    def test_run_online_rnn_training(self):
        ...
        config.batch_size = 2
        srsac = A.SRSAC(dummy_env, config=config, q_function_builder=RNNCriticBuilder(),
    ...
    def test_compute_eval_action(self):
        ...
        action = srsac.compute_eval_action(state)

        assert action.shape == dummy_env.action_space.shape

    def test_parameter_range(self):
        with pytest.raises(ValueError):
            A.SRSACConfig(tau=1.1)
        with pytest.raises(ValueError):
            A.SRSACConfig(tau=-0.1)
        with pytest.raises(ValueError):
            A.SRSACConfig(gamma=1.1)
        with pytest.raises(ValueError):
            A.SRSACConfig(gamma=-0.1)
        with pytest.raises(ValueError):
            A.SRSACConfig(start_timesteps=-100)
        with pytest.raises(ValueError):
            A.SRSACConfig(environment_steps=-100)
        with pytest.raises(ValueError):
            A.SRSACConfig(gradient_steps=-100)
        with pytest.raises(ValueError):
            A.SRSACConfig(initial_temperature=-100)
        with pytest.raises(ValueError):
            A.SRSACConfig(replay_ratio=0)
        with pytest.raises(ValueError):
            A.SRSACConfig(reset_interval=0)
    ...
    def test_latest_iteration_state(self):
        ...
        srsac._q_function_trainer_state = {'q_loss': 0., 'td_errors': np.array([0., 1.])}
        srsac._policy_trainer_state = {'pi_loss': 1.}

        latest_iteration_state = srsac.latest_iteration_state
...
class TestEfficientSRSAC():
    def setup_method(self, method):
        nn.clear_parameters()

    def test_algorithm_name(self):
        dummy_env = E.DummyContinuous()
        srsac = A.EfficientSRSAC(dummy_env)

        assert srsac.__name__ == 'EfficientSRSAC'

    def test_discrete_action_env_unsupported(self):
        """Check that error occurs when training on discrete action env."""
        dummy_env = E.DummyDiscrete()
        with pytest.raises(Exception):
            A.EfficientSRSAC(dummy_env)
    ...
    def test_run_offline_training(self):
        ...
        batch_size = 5
        dummy_env = E.DummyContinuous()
        config = A.EfficientSRSACConfig(batch_size=batch_size)
        srsac = A.EfficientSRSAC(dummy_env, config=config)
    ...
    def test_run_online_rnn_training(self):
        ...
        dummy_env = E.DummyContinuous()
        config = A.EfficientSRSACConfig()
        # setting rnn related settings to default to avoid assertion and enable reaching train_online
        config.actor_unroll_steps = 1
        config.actor_burn_in_steps = 0
        config.critic_unroll_steps = 1
        config.critic_burn_in_steps = 0
        config.start_timesteps = 7
        config.batch_size = 2

        with pytest.raises(RuntimeError):
            srsac = A.EfficientSRSAC(dummy_env, config=config, q_function_builder=RNNCriticBuilder(),
                                     policy_builder=RNNActorBuilder())
            srsac.train_online(dummy_env, total_iterations=10)
    ...
    def test_parameter_range(self):
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(tau=1.1)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(tau=-0.1)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(gamma=1.1)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(gamma=-0.1)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(start_timesteps=-100)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(environment_steps=-100)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(gradient_steps=-100)
        with pytest.raises(ValueError):
            A.EfficientSRSACConfig(initial_temperature=-100)
    ...
    def test_latest_iteration_state(self):
        ...
        srsac._actor_critic_trainer_state = {'q_loss': 0., 'td_errors': np.array([0., 1.]), 'pi_loss': 1.}
