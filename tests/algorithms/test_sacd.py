...
class RNNCriticFunction(FactoredContinuousQFunction):
    # type declarations to type check with mypy
    # NOTE: declared variables are instance variable and NOT class variable, unless it is marked with ClassVar
    # See https://mypy.readthedocs.io/en/stable/class_basics.html for details

    def __init__(self, scope_name: str, num_factors: int):
        super(RNNCriticFunction, self).__init__(scope_name)
        self._lstm_state_size = num_factors
        self._h = None
        self._c = None
    ...
    def num_factors(self):
        return self._lstm_state_size


class TestSACD(object):
    def setup_method(self, method):
        nn.clear_parameters()

    def test_algorithm_name(self):
        dummy_env = E.DummyFactoredContinuous(reward_dimension=2)
        sacd = A.SACD(dummy_env)
    ...
    def test_run_offline_training(self):
        ...
        sacd = A.SACD(dummy_env, config=config)
    ...
    def test_run_online_rnn_training(self):
        ...
        class RNNCriticBuilder(ModelBuilder[QFunction]):
            def build_model(self, scope_name: str, env_info, algorithm_config, **kwargs):
                return RNNCriticFunction(scope_name, num_factors=3)  # 2+1

        dummy_env = E.DummyFactoredContinuous(reward_dimension=2)
        config = A.SACDConfig(reward_dimension=2)
        ...
        config.batch_size = 2
        sacd = A.SACD(dummy_env, config=config, q_function_builder=RNNCriticBuilder(), policy_builder=RNNActorBuilder())
    ...
    def test_parameter_range(self):
        with pytest.raises(ValueError):
            A.SACDConfig(tau=1.1)
        with pytest.raises(ValueError):
            A.SACDConfig(tau=-0.1)
        with pytest.raises(ValueError):
            A.SACDConfig(gamma=1.1)
        with pytest.raises(ValueError):
            A.SACDConfig(gamma=-0.1)
        with pytest.raises(ValueError):
            A.SACDConfig(start_timesteps=-100)
        with pytest.raises(ValueError):
            A.SACDConfig(environment_steps=-100)
        with pytest.raises(ValueError):
            A.SACDConfig(gradient_steps=-100)
        with pytest.raises(ValueError):
    ...
    def test_latest_iteration_state(self):
        ...
        sacd._q_function_trainer_state = {"q_loss": 0.0, "td_errors": np.array([0.0, 1.0])}
        sacd._policy_trainer_state = {"pi_loss": 1.0}

        latest_iteration_state = sacd.latest_iteration_state
