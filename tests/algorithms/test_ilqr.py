...
class TestiLQR(object):
    ...
    def test_compute_eval_action(self):
        ...
        ilqr_config = A.iLQRConfig(T_max=T, num_iterations=100)
        ilqr = A.iLQR(env, dynamics=dynamics, cost_function=cost_function, config=ilqr_config)

        # initial pose
        x0 = np.array([[2.0], [0.0]])
        lqr_action = lqr.compute_eval_action(x0)
        ilqr_action = ilqr.compute_eval_action(x0)
    ...
    def test_run_offline_training(self):
        ...
        with pytest.raises(Exception):
            ilqr = A.iLQR(env, dynamics=dynamics, cost_function=cost_function)
            ilqr.train_offline(env)

    def test_parameter_range(self):
        with pytest.raises(ValueError):
            A.iLQRConfig(T_max=-1)
        with pytest.raises(ValueError):
            A.iLQRConfig(num_iterations=-1)
