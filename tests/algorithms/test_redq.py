...
class TestREDQ(object):
    def setup_method(self, method):
        nn.clear_parameters()

    def test_algorithm_name(self):
        dummy_env = E.DummyContinuous()
        redq = A.REDQ(dummy_env)
    ...
    def test_run_online_training(self):
        ...
        redq.train_online(dummy_env, total_iterations=10)
    ...
    def test_run_offline_training(self):
        ...
        redq.train_offline(buffer, total_iterations=10)
    ...
    def test_run_online_rnn_training(self):
        ...
        config.batch_size = 2
        redq = A.REDQ(dummy_env, config=config, q_function_builder=RNNCriticBuilder(), policy_builder=RNNActorBuilder())
    ...
    def test_compute_eval_action(self):
        ...
        action = redq.compute_eval_action(state)

        assert action.shape == dummy_env.action_space.shape

    def test_parameter_range(self):
        with pytest.raises(ValueError):
            A.REDQConfig(tau=1.1)
        with pytest.raises(ValueError):
            A.REDQConfig(tau=-0.1)
        with pytest.raises(ValueError):
            A.REDQConfig(gamma=1.1)
        with pytest.raises(ValueError):
            A.REDQConfig(gamma=-0.1)
        with pytest.raises(ValueError):
            A.REDQConfig(start_timesteps=-100)
        with pytest.raises(ValueError):
            A.REDQConfig(environment_steps=-100)
        with pytest.raises(ValueError):
            A.REDQConfig(gradient_steps=-100)
        with pytest.raises(ValueError):
            A.REDQConfig(initial_temperature=-100)
        with pytest.raises(ValueError):
            A.REDQConfig(G=-1)
        with pytest.raises(ValueError):
            A.REDQConfig(M=-1)
    ...
    def test_latest_iteration_state(self):
        ...
        redq._q_function_trainer_state = {"q_loss": 0.0, "td_errors": np.array([0.0, 1.0])}
        redq._policy_trainer_state = {"pi_loss": 1.0}

        latest_iteration_state = redq.latest_iteration_state
