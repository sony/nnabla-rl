...
@dataclass
class MMEVTrainerConfig(SquaredTDVFunctionTrainerConfig):
    pass


class MMEVTrainer(SquaredTDVFunctionTrainer):
    ...
    def __init__(self,
                 ...
                 temperature: AdjustableTemperature,  # value temperature alpha_q
                 env_info: EnvironmentInfo,
                 config: MMEVTrainerConfig = MMEVTrainerConfig()):
        ...
        super(MMEVTrainer, self).__init__(train_functions, solvers, env_info, config)
    ...
    def _compute_target(self, training_variables: TrainingVariables):
        ...
        # -min log_pi = max -log_pi
        offset = NF.top_k_data(-log_pi, k=1, reduce=True, base_axis=0)
        # Use offset only when the negative entropy is below 0
        # See the author's code: https://github.com/seungyulhan/mme/
        offset = NF.maximum_scalar(RNF.expand_dims(offset, axis=1), 0.0)
        return target_q + self._alpha_q * (log_pi + offset)
