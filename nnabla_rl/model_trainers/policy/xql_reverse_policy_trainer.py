...
@dataclass
class XQLReversePolicyTrainerConfig(TrainerConfig):
    beta: float = 1.0

    def __post_init__(self):
        super(XQLReversePolicyTrainerConfig, self).__post_init__()


class XQLReversePolicyTrainer(ModelTrainer):
    """EXtreme Q-learning style (w/ reverse KL-divergence) Policy Trainer."""

    # type declarations to type check with mypy
    # NOTE: declared variables are instance variable and NOT class variable, unless it is marked with ClassVar
    # See https://mypy.readthedocs.io/en/stable/class_basics.html for details
    _q_functions: Sequence[QFunction]
    _config: XQLReversePolicyTrainerConfig
    _pi_loss: nn.Variable
    _prev_policy_rnn_states: Dict[str, Dict[str, nn.Variable]]
    _prev_behavior_policy_rnn_states: Dict[str, Dict[str, nn.Variable]]
    ...
    def __init__(
        ...
        behavior_policy: StochasticPolicy,
        q_functions: Sequence[QFunction],
        env_info: EnvironmentInfo,
        config: XQLReversePolicyTrainerConfig = XQLReversePolicyTrainerConfig(),
    ):
        ...
        self._prev_behavior_policy_rnn_states = {}
        self._prev_q_rnn_states = {}
        for model in convert_to_list_if_not_list(models):
            self._prev_behavior_policy_rnn_states[model.scope_name] = {}
            self._prev_q_rnn_states[model.scope_name] = {}
        super(XQLReversePolicyTrainer, self).__init__(models, solvers, env_info, config)
        ...
        **kwargs,
        ...
        for t, b in zip(training_variables, batch):
            ...
            for q_function in self._q_functions:
                ...
                for state_name in t_rnn_states.keys():
                    set_data_to_variable(t_rnn_states[state_name], b_rnn_states[state_name])
            if self._behavior_policy.is_recurrent() and self._behavior_policy.scope_name in batch.rnn_states.keys():
                b_rnn_states = b.rnn_states[self._behavior_policy.scope_name]
                t_rnn_states = t.rnn_states[self._behavior_policy.scope_name]
                for state_name in t_rnn_states.keys():
                    set_data_to_variable(t_rnn_states[state_name], b_rnn_states[state_name])
    ...
    def _build_one_step_graph(self, models: Sequence[Model], training_variables: TrainingVariables, ignore_loss: bool):
        ...
        for policy in models:
            ...
            action, log_pi = policy_distribution.sample_and_compute_log_prob()

            prev_rnn_states = self._prev_behavior_policy_rnn_states[policy.scope_name]
            with rnn_support(
                self._behavior_policy, prev_rnn_states, train_rnn_states, training_variables, self._config
            ):
                behavior_distribution = self._behavior_policy.pi(training_variables.s_current)
            log_mu = behavior_distribution.log_prob(action)
            ...
            for q_function in self._q_functions:
                with rnn_support(q_function, prev_rnn_states, train_rnn_states, training_variables, self._config):
                    q_values.append(q_function.q(training_variables.s_current, action))
            self._prev_q_rnn_states[policy.scope_name] = prev_rnn_states
            min_q = RNF.minimum_n(q_values)

            return self._config.beta * (log_pi - log_mu) - min_q
    ...
    def _setup_training_variables(self, batch_size):
        ...
        if self._behavior_policy.is_recurrent():
            shapes = self._behavior_policy.internal_state_shapes()
            rnn_state_variables = create_variables(batch_size, shapes)
            rnn_states[self._behavior_policy.scope_name] = rnn_state_variables
