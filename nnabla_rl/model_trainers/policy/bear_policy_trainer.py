...
class AdjustableLagrangeMultiplier(Model):
    ...
    def __init__(self, scope_name, initial_value=None):
        super(AdjustableLagrangeMultiplier, self).__init__(scope_name=scope_name)
        if initial_value:
        ...
        with nn.parameter_scope(scope_name):
            self._log_lagrange = nn.parameter.get_parameter_or_create(
...
class BEARPolicyTrainer(ModelTrainer):
    ...
    def _repeat_state(self, s_var: nn.Variable, batch_size: int) -> nn.Variable:
        ...
        s_hat = NF.reshape(s_hat, shape=(batch_size * self._config.num_mmd_actions, s_var.shape[-1]))
        return s_hat
    ...
    def _build_one_step_graph(self, models: Sequence[Model], training_variables: TrainingVariables, ignore_loss: bool):
        ...
        for policy in models:
            sampled_actions = self._vae.decode_multiple(
                z=None, decode_num=self._config.num_mmd_actions, state=training_variables.s_current
            )
            ...
            if self._config.mmd_type == "gaussian":
                ...
                raise ValueError("Unknown mmd type: {}".format(self._config.mmd_type))
        ...
        self._lagrange_loss += (
            0.0 if ignore_loss else -NF.mean(-q_min + self._lagrange() * (mmd_loss - self._config.epsilon))
        )
