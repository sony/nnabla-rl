...
class GAILDiscriminator(RewardFunction):
    ...
    def r(self, s_current: nn.Variable, a_current: nn.Variable, s_next: nn.Variable) -> nn.Variable:
        ...
        with nn.parameter_scope(self.scope_name):
            h = NPF.affine(h, n_outmaps=100, name="linear1", w_init=RI.GlorotUniform(h.shape[1], 100))
            h = NF.tanh(x=h)
            h = NPF.affine(h, n_outmaps=100, name="linear2", w_init=RI.GlorotUniform(h.shape[1], 100))
            h = NF.tanh(x=h)
            h = NPF.affine(h, n_outmaps=1, name="linear3", w_init=RI.GlorotUniform(h.shape[1], 1))
